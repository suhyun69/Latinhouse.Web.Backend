...
# Create$${CAP_DOMAIN}WebRequest
cat <<EOF > $BASE_DIR/adapter/in/web/request/Create${CAP_DOMAIN}WebRequest.java
...
@AllArgsConstructor
@NoArgsConstructor
public class Create${CAP_DOMAIN}WebRequest {
...
package com.latinhouse.backend.${DOMAIN_NAME}.adapter.in.web;

import com.latinhouse.backend.${DOMAIN_NAME}.adapter.in.web.request.Create${CAP_DOMAIN}WebRequest;
import com.latinhouse.backend.${DOMAIN_NAME}.adapter.in.web.request.Update${CAP_DOMAIN}WebRequest;
import com.latinhouse.backend.${DOMAIN_NAME}.adapter.in.web.response.${CAP_DOMAIN}WebResponse;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Create${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Find${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Update${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.request.Create${CAP_DOMAIN}AppRequest;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.request.Update${CAP_DOMAIN}AppRequest;
...
public class ApiV1${CAP_DOMAIN}Controller {
    ...
    private final Create${CAP_DOMAIN}UseCase create${CAP_DOMAIN}UseCase;
    ...
    @PostMapping
    @Operation(summary = "Create ${CAP_DOMAIN}", description = "Create ${CAP_DOMAIN}")
    public ResponseEntity<${CAP_DOMAIN}WebResponse> create(@Valid @RequestBody Create${CAP_DOMAIN}WebRequest webReq) {

        Create${CAP_DOMAIN}AppRequest appReq = Create${CAP_DOMAIN}AppRequest.builder()
                .email(webReq.getEmail())
                .password(webReq.getPassword())
                .build();
        ${CAP_DOMAIN}WebResponse response = new ${CAP_DOMAIN}WebResponse(create${CAP_DOMAIN}UseCase.create(appReq));
...
# Create${CAP_DOMAIN}AppRequest
cat <<EOF > $BASE_DIR/port/in/request/Create${CAP_DOMAIN}AppRequest.java
...
@Value
@Builder
public class Create${CAP_DOMAIN}AppRequest {
    String email;
    String password;
}
EOF
...
# Create${CAP_DOMAIN}UseCase
cat <<EOF > $BASE_DIR/port/in/Create${CAP_DOMAIN}UseCase.java
package ${PACKAGE_NAME}.${DOMAIN_NAME}.port.in;

import ${PACKAGE_NAME}.${DOMAIN_NAME}.port.in.request.Create${CAP_DOMAIN}AppRequest;
import ${PACKAGE_NAME}.${DOMAIN_NAME}.port.in.response.${CAP_DOMAIN}AppResponse;

public interface Create${CAP_DOMAIN}UseCase {
    ${CAP_DOMAIN}AppResponse create(Create${CAP_DOMAIN}AppRequest appReq);
}
EOF
...
package com.latinhouse.backend.${DOMAIN_NAME}.application.service;

import com.latinhouse.backend.${DOMAIN_NAME}.domain.${CAP_DOMAIN};
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Create${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Find${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.Update${CAP_DOMAIN}UseCase;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.request.Create${CAP_DOMAIN}AppRequest;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.request.Update${CAP_DOMAIN}AppRequest;
import com.latinhouse.backend.${DOMAIN_NAME}.port.in.response.${CAP_DOMAIN}AppResponse;
import com.latinhouse.backend.${DOMAIN_NAME}.port.out.Create${CAP_DOMAIN}Port;
import com.latinhouse.backend.${DOMAIN_NAME}.port.out.Read${CAP_DOMAIN}Port;
import com.latinhouse.backend.${DOMAIN_NAME}.port.out.Update${CAP_DOMAIN}Port;
import lombok.RequiredArgsConstructor;
...
@Service
@RequiredArgsConstructor
public class ${CAP_DOMAIN}Service implements Create${CAP_DOMAIN}UseCase
        , Find${CAP_DOMAIN}UseCase
        , Update${CAP_DOMAIN}UseCase
{
    ...
    @Override
    public ${CAP_DOMAIN}AppResponse create(Create${CAP_DOMAIN}AppRequest appReq) {
...
public interface Create${CAP_DOMAIN}Port {
    ${CAP_DOMAIN} create(Create${CAP_DOMAIN}AppRequest appReq);
}
EOF
        ...
        , Update${CAP_DOMAIN}Port
{
    ...
    @Override
    public ${CAP_DOMAIN} create(Create${CAP_DOMAIN}AppRequest appReq) {
