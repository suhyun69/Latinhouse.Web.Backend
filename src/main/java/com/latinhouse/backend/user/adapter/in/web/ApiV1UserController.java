...
import com.latinhouse.backend.user.adapter.in.web.request.CreateUserWebRequest;
...
import com.latinhouse.backend.user.port.in.CreateUserUseCase;
...
public class ApiV1UserController {
    ...
    public ResponseEntity<UserWebResponse> create(@Valid @RequestBody CreateUserWebRequest webReq) {

        CreateUserAppRequest appReq = CreateUserAppRequest.builder()
                .email(webReq.getEmail())
                .password(webReq.getPassword())
                .build();
        UserWebResponse response = new UserWebResponse(createUserUseCase.create(appReq));
